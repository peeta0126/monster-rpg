...
## git 에 함께 들어갑니다 (배포되지는 않습니다)

이 폴더는 `public/` 밖이라 Vite 가 `dist/` 로 복사하지 않습니다. 즉 **저장소에는 남고
배포물에는 안 실립니다.**

처음엔 `.gitignore` 로 빼뒀는데, 그 상태에서 배경 마스터를 잃는 사고가 실제로 났습니다
(새로 클론하면 비어 있으니 "복구"한다며 비슷하게 생긴 다른 그림을 넣었고, 그걸로 WebP 를
다시 구웠습니다. 비주얼 회귀도 못 잡았습니다). 마스터가 저장소에 없으면 무엇이 정본인지
확인할 방법이 없다는 게 문제였습니다. 지금은 커밋하고 해시로 지킵니다.
