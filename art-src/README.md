...
없습니다. 지금 레시피가 읽는 마스터는 공방 배경 하나뿐인데, 그건 여기가 아니라
`public/assets/housing/housing_bg.png` 에 **커밋된 채로** 있습니다.

여기가 gitignore 대상이라 새로 클론하면 비어 있고, 그 상태에서 공방 배경 마스터를
잃는 사고가 실제로 났습니다(비슷하게 생긴 다른 그림을 마스터로 착각해 WebP 를 다시
구웠고, 비주얼 회귀도 못 잡았습니다). 그래서 그 하나는 커밋되는 자리로 옮기고
`tests/workshopBackground.test.mjs` 가 SHA256 으로 지킵니다.

새 마스터를 추가할 때는 잃어도 되는 것만 여기 두고, 잃으면 안 되는 건 커밋되는 곳에
두고 해시로 지키세요.
