...
다섯 시트의 발끝이 칸 바닥에서 제각각이다. 원화를 다시 굽지 않고 표로 흡수한다 —
칸마다 "발이 닿는 줄"을 재서 `shared/playerSprite.PLAYER_SHEET_METRICS` 에 적어 두고,
그리는 기준점(`spriteOriginY`)과 발밑 판정(`campCollision.playerBodyOffset`)이 같이
그 값을 읽는다. 원화를 갈아끼웠으면 `node scripts/measure-player-sheets.mjs` 로 다시
재서 표를 고칠 것. 안 고치면 인물이 공중에 뜨고 방향을 바꿀 때마다 판정이 튄다.
