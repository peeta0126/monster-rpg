...
function verdict(floor: number, bare: number, under: number, std: number, imp: number): string {
  ...
  if (isBossFloor(floor)) {
    ...
    // ⚠️ **이 열에 상한을 걸지 않는다.** 지금 다섯 층 모두 100% 이고, 그걸 내리려고
    // 두 가지를 다 재 봤다(2026-08-21).
    //
    //   · 보스를 올린다  — 각인 열이 88~92 를 지날 때 정규 열은 이미 0~15% 다.
    //                      이 근처의 승률 곡선은 능력치 1% 당 5포인트로 서 있어서,
    //                      폭 20짜리 띠 둘에 ×1 파티와 ×2 파티를 같이 넣을 수 없다.
    //   · 각인을 낮춘다  — 등급당 5%→4% 만 해도 **실제 판**의 50층 재도전이
    //                      3.5 → 6.6 회로 뛴다(scripts/sim/run.ts). 이 열은 최상급
    //                      장비 + 만렙 각인을 가정하는데 실제 파티는 거기 못 미쳐서,
    //                      각인을 깎으면 이 열은 거의 안 움직이고 실제 판만 무너진다.
    //
    // 즉 100% 는 "완비하면 이긴다"는 뜻이고, 그건 이 게임의 약속 그대로다. 진짜 벽은
    // 이 열이 아니라 run.ts 의 보스 재도전 횟수로 본다(40층 3.1 · 50층 3.5).
