...
if (apiBase) {
  ...
  console.log(`[1/4] server-config.json → ${apiBase}`);
} else {
  const current = JSON.parse(fs.readFileSync(CONFIG, "utf8")).apiBase;
  console.log(`[1/4] server-config.json 그대로 (apiBase: ${current || "비어 있음"})`);
}
