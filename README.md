...
| `TRUST_PROXY` | 앞에 선 프록시의 **겹 수**입니다. 혼자 쓸 때는 `0`, 터널로 내보낼 때는 `2`. 모자라면 모든 접속이 같은 IP 로 보여 요청 제한이 전원 공용이 됩니다. |
...
루트에서 부를 때는 `npm --prefix server run migrate` 를 씁니다. `npm run` 이어야 작업 디렉터리가
`server/` 로 잡혀 `.env` 와 스키마를 찾습니다 — `npm --prefix server exec prisma ...` 는 못 찾습니다.
...
들어오는 길은 아이디·비밀번호 하나입니다. 예전의 「바로 시작」(익명 계정)은 없앴으므로,
백엔드 없이 `npm run dev`만 띄우면 로그인 화면에서 더 나아갈 수 없습니다.

### 둘 이상이 같이 개발할 때

**각자 자기 PC 에서 서버를 띄웁니다.** 한쪽이 켜 줘야 다른 쪽이 볼 수 있는 구조가 아닙니다.
`server/setup.ps1`(PowerShell) 또는 `server/setup.sh` 를 한 번 돌리면 의존성·`.env`·DB·빌드가
한 번에 끝나고, **그 기기만의 `JWT_SECRET` 과 `ADMIN_SECRET` 이 랜덤으로 생성됩니다.**
`server/.env` 와 `dev.db` 는 저장소에 올라가지 않으니 서로 주고받을 필요도, 맞출 필요도 없습니다.

그래서 계정과 세이브는 기기마다 따로입니다. 같은 계정으로 두 PC 를 오가려면 한쪽을
터널로 내보내고 둘 다 그 주소를 봐야 합니다 (docs/SERVING.md).

`git pull` 로 마이그레이션이 새로 들어온 날에는 DB 를 따라 올려야 합니다.

```bash
npm --prefix server run migrate
```

깜빡해도 서버가 기동할 때 안 들어간 마이그레이션을 짚어 줍니다. 안 하면 그 표를 건드리는
요청부터 500 으로 죽는데, 화면에서는 "로그인이 안 된다" 로만 보입니다.
