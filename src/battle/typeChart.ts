 ...
 *   grass  → electric 2배 — 전기의 약점. 덤으로 풀에게 쓸 이유가 생긴다.
 *   ice    → poison   2배 — 독의 약점. 얼음도 때릴 상대가 둘로 늘어난다.
 *   poison → normal   2배 — 노말의 약점. 독에게도 풀 말고 다른 먹이가 생긴다.
 *
 * 결과로 fire → grass → electric → water → fire 가 물고 물리는 순환이 되고,
 * 그 바깥에 ice → poison → normal 이 붙는다. 어느 속성도 혼자 안전하지 않다.
 *
 * normal 은 **때리는 쪽 줄이 비어 있는 정체성을 유지한다**(상성이 없는 대신
 * 최상급 기술 위력이 제일 높다). 맞는 쪽에만 약점을 하나 줬다.
 */
