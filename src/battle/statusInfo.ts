 ...
 * 상태이상 표시의 단일 출처.
 *
 * 원래 BattlePage 와 BattleScene 이 각자 `{ paralysis: "마비", ... }` 표를 들고 있었다.
...
/**
 * 색은 밝은 토큰만 쓴다. 원래 독이 earth-500, 화상이 ember-600 이었는데 둘 다 어두운
 ...
 *
 * 원래 이름 앞에 이모지를 붙였다. 캔버스 로그는 픽셀 폰트로 그려지는데 그 폰트에
 * 이모지가 없어서 그 글자만 다른 서체로 떨어졌다. 그림이 필요한 DOM 자리는
