 ...
 * 이번 라운드의 행동 순서.
 *
 * 이 게임 속도는 굴림이 아니라 누적이다(battleUtils.tickSpeedGauge). 누가 먼저
 * 움직이는지도, 누가 한 번 더 움직이는지도 턴이 시작되기 전에 이미 정해져 있다.
 * 정해져 있는데 화면이 안 알려 주면 그건 숨긴 거지 긴장이 아니다.
 *
 * 원래는 `▲ 선공 · 연속 3턴` 이라는 글자 한 줄이 전부였다. 옥토패스·브레이블리가 쓰는
 * 순서 띠를 그대로 가져왔다. 칩 세 개면 "내가 먼저, 적, 그리고 내가 또"가 한눈에 읽힌다.
 */
