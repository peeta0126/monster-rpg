import type { Monster } from "../types/game";
import {
  ember, tackle, vineWhip, waterGun,
  spark, thunderbolt, voltCrash, thunderStrike,
  iceBeam, blizzard,
  quickAttack, bodySlam,
  flamethrower, surf,
  aquaWhirl,
  crystalBurst,
} from "./moves";

export const monsters: Monster[] = [

  // ─── 불꽃 ──────────────────────────────────────────────────────────────────
  {
    id: "flameling",
    name: "플레미",
    type: "fire",
    maxHp: 120,
    attack: 30,
    defense: 20,
    speed: 25,
    moves: [tackle, ember],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 40,
  },
  {
    id: "burno",
    name: "버노",
    type: "fire",
    maxHp: 100,
    attack: 38,
    defense: 18,
    speed: 30,
    moves: [tackle, ember, flamethrower],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 48,
  },

  // ─── 물 ────────────────────────────────────────────────────────────────────
  {
    id: "aquabe",
    name: "아쿠비",
    type: "water",
    maxHp: 110,
    attack: 28,
    defense: 22,
    speed: 20,
    moves: [tackle, waterGun],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 35,
    evolutionStage: 1,
    evolutionChainId: "aqua",
    evolvesTo: "aquavern",
    evolvesAtLevel: 22,
  },
  {
    id: "aquavern",
    name: "아쿠사",
    type: "water",
    maxHp: 175,
    attack: 58,
    defense: 45,
    speed: 38,
    moves: [tackle, waterGun, surf, aquaWhirl],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 78,
    evolutionStage: 2,
    evolutionChainId: "aqua",
    evolvesFrom: "aquabe",
  },
  {
    id: "bubblet",
    name: "버블릿",
    type: "water",
    maxHp: 105,
    attack: 30,
    defense: 20,
    speed: 28,
    moves: [tackle, waterGun, surf],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 42,
  },

  // ─── 풀 ────────────────────────────────────────────────────────────────────
  {
    id: "leafy",
    name: "리피",
    type: "grass",
    maxHp: 115,
    attack: 29,
    defense: 21,
    speed: 23,
    moves: [tackle, vineWhip],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 38,
  },

  // ─── 전기 (모시 계열) ────────────────────────────────────────────────────────
  {
    id: "mossy",
    name: "모시",
    type: "electric",
    maxHp: 125,
    attack: 26,
    defense: 26,
    speed: 18,
    moves: [tackle, spark],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 44,
    evolutionStage: 1,
    evolutionChainId: "mossy",
    evolvesTo: "mossevo",
    evolvesAtLevel: 20,
  },
  {
    id: "mossevo",
    name: "모치",
    type: "electric",
    maxHp: 185,
    attack: 62,
    defense: 44,
    speed: 52,
    moves: [tackle, spark, thunderbolt, voltCrash],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 88,
    evolutionStage: 2,
    evolutionChainId: "mossy",
    evolvesFrom: "mossy",
    evolvesTo: "mossyfinal",
    evolvesAtLevel: 38,
  },
  {
    id: "mossyfinal",
    name: "모왕",
    type: "electric",
    maxHp: 240,
    attack: 95,
    defense: 62,
    speed: 80,
    moves: [spark, thunderbolt, voltCrash, thunderStrike],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 140,
    evolutionStage: 3,
    evolutionChainId: "mossy",
    evolvesFrom: "mossevo",
  },

  // ─── 얼음 (새 2종) ───────────────────────────────────────────────────────────
  {
    id: "crystafox",
    name: "크리샤",
    type: "ice",
    maxHp: 110,
    attack: 32,
    defense: 28,
    speed: 26,
    moves: [tackle, iceBeam],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 46,
  },
  {
    id: "frostorb",
    name: "프리로",
    type: "ice",
    maxHp: 130,
    attack: 24,
    defense: 38,
    speed: 14,
    moves: [tackle, iceBeam, crystalBurst],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 50,
  },

  // ─── 노말 ────────────────────────────────────────────────────────────────────
  {
    id: "nobi",
    name: "노비",
    type: "normal",
    maxHp: 118,
    attack: 28,
    defense: 24,
    speed: 22,
    moves: [tackle, quickAttack],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 38,
  },

  // ─── 더미 데이터 (미구현 - 추후 추가 예정) ────────────────────────────────────
  {
    id: "voltiny",
    name: "볼티",
    type: "electric",
    maxHp: 100, attack: 32, defense: 18, speed: 32,
    moves: [tackle, spark],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 40,
    isDummy: true,
  },
  {
    id: "zapbear",
    name: "잽베어",
    type: "electric",
    maxHp: 120, attack: 40, defense: 25, speed: 22,
    moves: [tackle, spark, thunderbolt],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 52,
    isDummy: true,
  },
  {
    id: "frostlet",
    name: "프로스틀릿",
    type: "ice",
    maxHp: 105, attack: 25, defense: 30, speed: 18,
    moves: [tackle, iceBeam],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 38,
    isDummy: true,
  },
  {
    id: "blizzwolf",
    name: "블리자울프",
    type: "ice",
    maxHp: 115, attack: 36, defense: 22, speed: 28,
    moves: [tackle, iceBeam, blizzard],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 50,
    isDummy: true,
  },
  {
    id: "fluffin",
    name: "플러핀",
    type: "normal",
    maxHp: 130, attack: 22, defense: 30, speed: 15,
    moves: [tackle, quickAttack],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 35,
    isDummy: true,
  },
  {
    id: "stonepup",
    name: "스톤퍼프",
    type: "normal",
    maxHp: 110, attack: 28, defense: 36, speed: 18,
    moves: [tackle, bodySlam],
    level: 1, exp: 0, expToNextLevel: 100,
    rewardExp: 42,
    isDummy: true,
  },
];
