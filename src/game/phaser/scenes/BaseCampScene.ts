...
export default class BaseCampScene extends Phaser.Scene {
  ...
  create() {
    ...
    // ── 왼쪽 나무 경계 (나무쪽에서 아래로) ─────────────────────────────────────────
    // 수직 x=204: y=750→992 (직각 코너 형성)
    seg(204, 750, 204, 992);
    // 트리 벽: 맵 상단~y=992 연속 (대각선 제거 → 직각으로 교체)
    seg(150, 0, 150, 992);
    seg(150, 992, 150, 1350);  // 우물 아래쪽 왼쪽 경계
    seg(150, 1350, 150, 2200); // 나무 경계 남쪽 계속 (아치 이후까지)

    // ── 상단 마을 경계 ─────────────────────────────────────────────────────────────
    seg(397, 0, 397, 750);     // 탑 동쪽 수직 경계 (맵 상단까지)
    seg(150, 750, 397, 750);   // 탑 왼쪽 나무 연결 수평 (갭 메꿈)
    seg(397, 750, 430, 750);   // 탑~여관 왼쪽 연결
    seg(843, 750, 1050, 750);  // 상단 수평 경계 (우) : 시계탑 하단 ~ 동쪽 외벽

    // ── 여관 건물 경계 (이미지 기준) ──────────────────────────────────────────────
    seg(843, 0, 843, 750);     // 시계탑 왼쪽 / 여관 오른쪽 경계
    seg(430, 570, 843, 570);   // 여관 상단 경계
    seg(430, 570, 430, 750);   // 여관 왼쪽 경계

    // ── 과일상점 서쪽 경계 확장 ────────────────────────────────────────────────────
    seg(570, 920, 733, 920);   // 과일상점 북벽 서쪽 확장
    seg(570, 920, 570, 1000);  // 과일상점 서쪽 수직

    // ── 우물 경계 (우물 접근 차단) ─────────────────────────────────────────────────
    seg(397, 992, 488, 992);   // 우물 북쪽 벽 (탑 수평 동쪽 연장)
    seg(488, 992, 488, 1170);  // 우물 동쪽 벽

    // ── 우물 아래 서쪽 (지붕 진입 차단) ────────────────────────────────────────────
    seg(347, 1170, 347, 1205); // 대각선 끝에서 아래로 연장
    seg(347, 1205, 488, 1205); // 하단 마감 (기존 seg(488,1205) 서쪽 연장)

    // ── 집/과일상점 북쪽 벽 + 양쪽 수직 연결 ────────────────────────────────────────
    seg(733, 1000, 843, 1000); // 북쪽 수평 벽
    seg(733, 1000, 733, 1117); // 북벽 ~ 상점 좌측 연결
    seg(843, 1000, 843, 1117); // 북벽 ~ 상점 우측 연결

    // ── 마을 동쪽 외벽 ─────────────────────────────────────────────────────────────
    seg(1050, 0, 1050, 1580);  // 마을 동쪽 경계 전체 (맵 상단 ~ 하단 통로 입구)

    // ── 하단 오른쪽 나무/덤불 경계 ────────────────────────────────────────────────
    seg(900, 1350, 1050, 1350); // 하단 오른쪽 나무 북쪽 경계

    // ── 하단 왼쪽 건물 (물레방아) ─────────────────────────────────────────────────
    seg(55, 1480, 320, 1480);  // 물레방아 건물 북쪽 경계
    seg(320, 1480, 320, 1760); // 물레방아 건물 동쪽 경계

    // ── 아치 아래 / 숲 진입로 경계 ────────────────────────────────────────────────
    // 아치 좌측 출구(820,1758) → 남서쪽 사선으로 트리 벽과 연결
    seg(820, 1758, 150, 2200); // 아치 출구에서 왼쪽 나무 경계로 이어지는 사선
    // 숲 하단 바닥 경계 (맵 과도한 하단 진입 차단)
    seg(150, 2200, 1536, 2200);
