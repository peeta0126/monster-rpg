...
export default class BaseCampScene extends Phaser.Scene {
  ...
  create() {
    ...
    // ── 경로 외곽 경계 (플레이어가 실제로 걷는 구간만) ─────────────────────────────
    seg(150,  992, 204,  992);   // 탑 통로 왼쪽 위 닫기
    seg(397,  992, 1050, 992);   // 탑 통로 오른쪽 ~ 동쪽 외벽 상단 닫기
    seg(150,  992, 150, 1350);   // 경로 왼쪽 외벽
    seg(150, 1350, 134, 1378);   // 왼쪽 외벽 → 우물 연결

    // ── 경로 동쪽 외벽 (y=992부터 숲 연결 지점까지) ─────────────────────────────────
