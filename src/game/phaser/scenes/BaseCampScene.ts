...
export default class BaseCampScene extends Phaser.Scene {
  ...
  create() {
    ...
    seg(150, 0, 150, 992);
    seg(150, 992, 150, 1350);  // 왼쪽 경계
    seg(150, 1350, 134, 1378); // 나무 경계 → 우물 경계 연결
    ...
    seg(733, 1000, 843, 1000);
    seg(733, 1000, 733, 1117);
    seg(843, 1000, 843, 1117);

    // ── 마을 동쪽 외벽 ─────────────────────────────────────────────────────────────
    seg(1050, 0, 1050, 1580);

    // ── 우물 및 하단 구역 경계 (유저 지정 좌표) ────────────────────────────────────
    seg(134, 1378, 457, 1464);   // 우물 상단 (대각선)
    seg(457, 1464, 574, 1464);   // 오크통 구간 (수평)
    seg(574, 1464, 506, 1863);   // 아래로 내려가는 구간
    seg(506, 1863, 106, 1863);   // 하단 왼쪽 수평
    seg(106, 1863, 106, 1964);   // 왼쪽 수직
    seg(106, 1964, 529, 1969);   // 수평
    seg(529, 1969, 529, 2289);   // 수직 하단
    seg(529, 2289, 1250, 2289);  // 하단 수평
    seg(1250, 2289, 1250, 1985); // 오른쪽 수직 (위로)
    seg(1250, 1985, 1527, 1987); // 오른쪽 수평
    seg(1527, 1987, 1527, 1580); // 오른쪽 수직 위로 (동쪽 외벽 연결)
