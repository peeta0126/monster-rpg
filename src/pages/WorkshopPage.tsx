...
import { CraftingModal } from "../components/workshop/CraftingModal";
import { usePlayerStore } from "../store/playerStore";
import type { CraftingStationType } from "../types/crafting";
import { QUALITY_COLOR, QUALITY_LABEL } from "../utils/crafting";
...
const CRAFTING_STATIONS = [
  {
    ...
    label: "아티팩트 제작대",
    type: "artifact" as CraftingStationType,
    x: 22,   // 좌측 제작대 — 필요시 조정
    y: 85,
    radius: 10,
  },
  {
    ...
    label: "연금술 제작대",
    type: "potion" as CraftingStationType,
    x: 80,   // 우측 제작대 — 필요시 조정
    y: 35,
    radius: 10,
  },
...
function getPlayerImage(dir: Direction, frame: 0 | 1 | 2): string {
  if (frame === 0) return `/assets/basecamp/player-${dir}.png`;
  return `/assets/basecamp/player-${dir}-${frame}.png`;
}
...
export default function WorkshopPage() {
  ...
  // ── 모달 상태 ────────────────────────────────────────────────────────────────
  const [activeStation, setActiveStation] = useState<CraftingStationType | null>(null);
  const activeStationRef = useRef<CraftingStationType | null>(null);
