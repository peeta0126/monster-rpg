 ...
 *
 * 고친 뒤에는 `npm run design:collision` 으로 원화 위에 겹쳐 찍어 눈으로 봐라.
...
export const WALL_SEGMENTS: WallSegment[] = [
  ...
  // 바닥이라 아랫변으로 긋는다. 윗변으로 그으면 탁자 위를 걸어 다닌다.
 ...
 * 정사각형을 늘어놓아야 한다. 그건 그때 가서 늘리면 된다.
 */
 ...
 *
 * 원래는 좌표 한 점으로만 충돌을 봤다. 스프라이트는 128px(스테이지 폭의 약 7%)라,
