 ...
 * 값어치가 안 느껴진다.
 *
 * 보상을 고르는 기준은 하나다. **그 시점에 스스로 구하려면 몇 번을 다녀와야 하는지 세어보고,
 * 그 수가 크지 않으면 보상으로 쓰지 않는다.** 5층에 주는 강화석이 그렇다 — 11층 위 전투와
 * 고대 숲에만 있어서 그 시점에는 구할 길이 아예 없다.
 */
export type QuestReward =
   ...
   *
   * 레벨은 **받는 시점 파티 최고 레벨에서 이만큼 뺀 값**이다. 고정 레벨로 주면 일찍 온
 ...
 * 보상 한 줄을 화면 표기로.
 *
 * `hideMonster` 는 아직 안 받은 것을 미리 보여줄 때 쓴다 — 뭘 받을지 미리 알면 10층
 * 완료 대사가 죽는다.
 */
