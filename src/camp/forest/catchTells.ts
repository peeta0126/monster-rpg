 ...
 *
 * 원래 상대 손이 완전 균등이라 세 버튼 기대값이 전부 0.44 로 수학적으로 같았다.
 * 헷갈리는 버튼이 아니라 진짜로 똑같은 버튼 세 개였고, 그게 숲에서 제일 자주 누르는
 * 버튼이었다.
 *
 * 단위가 종이 아니라 속성인 이유는, 종은 스물여섯이라 못 외우고 속성은 여덟이라
 * 외워져서다. 그리고 속성 칩은 이미 카드에 붙어 있어서 표를 새로 보여 줄 자리를
 * 만들 필요가 없다.
 *
 * ⚠️ 버릇은 **주속성만** 본다(tellTypeOf). 이중 속성이 둘 다 버릇을 갖는다면 손 분포를
 *    섞어야 하는데, 그러면 카드에 적힌 한 줄("주로 바위를 낸다")이 거짓말이 된다.
 *    보여 준 대로 굴러야 정보를 사는 값이 성립한다.
 *
 ...
 *
 *   모르고 아무거나        0.440   (어떤 편향에서도 안 변한다. 세 손의 평균)
 ...
 * 이 몬스터가 이번에 내는 수.
 *
 * ⚠️ rng() 를 정확히 한 번만 쓴다. 시도 시드(attemptRng)는 손 다음에 최종 포획
 ...
 * 버릇을 얼마나 열어 줄지.
 *
 * 항상 보여 주면 가위바위보가 정답 맞히기가 된다. 그래서 이 정보는 사는 것이다.
 * 낮은 소란도(정찰)와 도감이 그 값을 치른다. 처음 보는 몬스터의 버릇을 못 읽는 건
 * 불친절이 아니라 규칙이다. 친절해 보이려고 마음대로 열지 마라.
 */
