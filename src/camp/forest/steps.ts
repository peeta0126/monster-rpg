 ...
 * 걸음 사건 한 벌.
 *
 * 숲은 지도가 아니라 걸음이다. 원래는 노드 그래프를 미리 깔고 그 위를 걸었는데,
...
export interface StepDef {
  ...
  /** 사건 패널 제목 */
  title: string;
  /** 정찰이 제일 좋을 때 보여 주는 한 줄. 뭘 얻는지만 적는다 */
  hint: string;
  tier: StepTier;
...
/** 길 이름. 갈림길 카드의 제목이고, 사건 정체를 흘리지 않는 중립적인 이름만 쓴다 */
 ...
 * 기대 수확이 흔적의 1/3 이었다. 비싼 길이 언제나 손해였다는 얘기다.
 ...
 * 이 사건이 내놓는 재료.
 *
 * 화면(ForestRunView)이랑 시뮬(gameModel)이 각자 굴리고 있었다. 한쪽에만 희귀 풀과
 ...
 *
 * 원래는 무조건 +30 이었다. 소란 예산의 24% 를 한 번에 태우는데 놓치는 건 상당 부분
