 ...
 * 원정 저장.
 *
 * 규칙은 runStore 에 있고 여기는 그걸 디스크에 얹는 일만 한다. 저장하는 순간은 둘뿐.
 * 런이 바뀔 때마다(걸음 안의 시도 횟수까지 포함), 그리고 정산 화면에 들어갈 때다.
 * 정산을 저장 안 하면 제일 아까운 순간에 새로고침이 수확을 지운다.
 *
...
/** 사생활 보호 모드는 접근 자체가 던진다. 그것 때문에 숲이 안 열리면 안 된다 */
 ...
 * 구역과 가방만 긁어서 자진 귀환과 똑같이 100% 정산으로 보낸다.
 *
