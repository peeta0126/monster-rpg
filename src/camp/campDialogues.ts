...
export interface QuestDef {
   ...
   *
   * `questDone` 이 있는 건 새 퀘스트마다 스토리 플래그를 만들지 않으려고 그런 거다.
...
export interface DialogueEntry {
  /**
   * 이 대사를 가리키는 이름표. 세이브에 이 문자열이 그대로 들어가니까, 한 번 정한
   * 뒤에는 바꾸지 마라. 바꾸면 이미 본 사람이 그 대사를 다시 본다.
   *
...
export const ORION_MOTHERS_MEDICINE_QUEST: QuestDef = {
  ...
  acceptLines: [
    "부탁이 하나 있다.",
    "약을 지어보려 한다. 슬라임 추출물이 두개 필요해.",
    "…솔직히 말하마. 이게 들을지 나도 모른다.",
    "원인을 모르는데 약을 짓는다는 게 말이 안 되지. 그래도 가만히 있는 것보단 낫지 않겠느냐.",
    "낫게는 못 해도, 조금 편하시게는 할 수 있을 게다. 그동안 네가 탑을 오를 시간을 벌고.",
  ],
 ...
 *
 * 5층 이야기 대사가 "슬슬 맨몸으로는 안 된다"고 짚는데 그걸 강제하는 게 없었다.
 * 보상이 강화석인 건 일부러 그런 건데, 11층 위 전투랑 고대 숲에만 있어서 이 시점엔
 * 구할 길이 아예 없다. 모루가 그때부터 돈다.
 */
...
/**
 * 10층 · 오리온. 이야기가 "나도 10층에서 멈췄다"를 말하는 자리고, 그 대사를 퀘스트가 받는다.
 *
 * 몬스터를 주는 유일한 퀘스트다. 리피인 이유는 셋이다.
 *   · 로스터에서 풀은 리피 하나뿐이고, 리피가 나오는 얕은 숲은 레벨 상한이 8이다. 숲이
 *     파티 최고 레벨을 천장으로 쓰니까, 10층을 넘긴 파티는 쓸 만한 레벨로 잡을 수단이
 *     영영 없다.
 ...
 * 못 만든다. 35층 관문이 물약 운용을 전제로 한 층이라 그 자리를 메운다.
 */
...
export const BAROS_CHANGE_GEAR_QUEST: QuestDef = {
  ...
  acceptLines: [
    "40층. 인정한다.",
    "네 장비를 봤다. 만든 지 오래됐군.",
    "하나라도 좋다. 더욱 성장해라. 그 전엔 그 위를 못 본다.",
  ],
  progressLines: ["성장에는 여러 방법이 있지. 모루를 써라. 안 쓰는 장비를 분해하면 강화석이 나온다."],
  completeLines: [
    "됐다. 그게 45층을 넘기는 물건이다.",
    "강화석 열개다. 내가 이십 년 모은 거다.",
...
/**
 * 퀘스트 전부. 진행 순서대로 적는다. 한 사람이 한 번에 하나만 내놓을 때 뭘 먼저
 * 내놓을지가 이 순서고, 퀘스트 로그의 표시 순서이기도 하다.
 *
...
export const ORION_DIALOGUES: DialogueEntry[] = [
  ...
  // 어머니의 약 (quest_baros_done + 3층 도달 이후). 미수락/진행중/완료는 QuestDef 가 처리하고, 이 lines 는 완료 후 필터
...
export const BAROS_DIALOGUES: DialogueEntry[] = [
  ...
  // 원래 여기서 포획·육성·제작·분해를 19줄에 몰아넣었다. 그 시점에 못 하는 일까지
  ...
  // 첫 사냥 (met_baros 이후). 미수락/진행중/완료는 QuestDef 가 처리하고, 이 lines 는 완료 후 필터
  {
  ...
  // 4-2. 첫 포획 후. 파티랑 재료 이야기는 실제로 잡아 온 다음에 한다
  {
  ...
  // 4-3. 5층 도달. 장비 이야기는 맨몸이 슬슬 안 먹히는 이 시점에 한다
  {
    id: "baros_floor_5",
    requires: "floor_5",
    lines: [
 ...
 * 지금 이 사람이 내놓을 퀘스트 하나.
 *
 * 한 번에 하나만 내놓는다. 엔딩까지 본 옛 세이브를 열면 뒤쪽 퀘스트 조건이 한꺼번에
 * 채워지는데, 다 내놓으면 로그가 한 번에 여덟 줄로 불어난다. 낮은 층 것부터 하나씩
 * 흐르면 "돌아온 사람에게 마을이 밀린 부탁을 한다"로 읽힌다.
 */
 ...
 *      이 하나만 이야기보다 앞이다. 손에 든 걸 건네러 온 사람을 돌려세우지 않는다.
 *   2. 아직 안 본 이야기 대사. 조건을 만족하는 것 중 가장 앞선 것부터 하나씩
 *   3. 미수락 퀘스트 (수락 대사)
 *   4. 진행중 퀘스트 (독촉 대사)
 *   5. 잡담
 *
 * 원래는 2번이 "조건을 만족하는 것 중 마지막"이었다. 플래그는 쌓이기만 하니 진행할수록
...
export function resolveNpcInteraction(
  ...
  // 4.5) 메우는 줄. 이야기가 하나도 안 열린 사람에게만 쓴다.
  //      바로스의 "이장 영감한테 먼저 가봐라"가 그렇다. 아직 이야기가 시작도 안 됐는데
  //      날씨 얘기를 시킬 수는 없다. 이야기가 하나라도 열렸으면 그 줄은 역할이 끝난
  //      거라 잡담으로 넘어간다.
