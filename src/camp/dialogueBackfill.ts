 ...
 * 엔딩까지 본 사람이 첫 만남 대사부터 다시 듣는다는 얘기다.
 *
 * 그래서 조건을 만족하는 대사는 전부 봤다고 찍되, 사람마다 마지막 하나는 남긴다.
 * 그 마지막 하나가 곧 지금 코드가 다음 대화에서 보여줄 그 대사라, 옛 세이브의 다음
 * 한 번은 예전과 똑같이 흐르고 그 뒤부터 잡담으로 넘어간다. 이어붙인 티가 안 난다.
 *
 * ⚠️ 부르는 쪽 조건. 기록이 아예 없을 때(undefined)만 불러라. 빈 배열은 "새로 시작해서
 * 아직 아무것도 안 봤다"는 뜻이라 손대면 안 된다. 세이브 정규화는 저장할 때마다도
 * 도니까, 그 구분을 놓치면 새 플레이어가 매 저장마다 전 대사를 본 것으로 찍힌다.
 */
