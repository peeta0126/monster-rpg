 ...
 * 전투 진행 설정.
 *
 * 로그가 한 줄마다 Q 를 기다린다. 엔딩까지 8,000번쯤 되니 자동 진행이 없으면
 * 손가락이 먼저 지친다(Handoff 6장 1번). 그래서 기본이 자동이다. 설정을 뒤져서
 * 켠 사람만 편해지면 안 되니까. 기다리기 싫으면 아무 때나 Q 로 넘기면 되고,
 * 누르고 있으면 더 빨리 흐른다(BattleScene.registerInput).
 *
 * 이미 저장된 설정은 그대로 둔다. persist 가 저장값을 기본값 위에 덮으니까
