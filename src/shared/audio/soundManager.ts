 ...
 * 조용히 넘어간다. BGM 여섯 곡은 들어왔지만 효과음은 아직 없고, 그 상태로 게임이
 * 정상 동작해야 한다.
 */
 ...
 *
 * 그래서 **먼저 틀어 보고, 막히면 기억해 뒀다가 첫 클릭에 다시 시도한다.** 처음부터
 * 잠금 플래그로 막아 두면, 이미 이 사이트에서 소리를 낸 적이 있어 브라우저가 허용해
 ...
 * 이 화면의 곡을 건다.
 *
 * **이미 그 곡이 나오고 있으면 아무것도 하지 않는다.** 마을 ↔ 가방 ↔ 내 몬스터처럼
 * 같은 곡을 쓰는 화면을 오갈 때 매번 처음으로 되감기면 안 된다. 곡이 바뀔 때만
 * 앞 곡을 줄이면서 뒷 곡을 올린다.
 */
export function playBgm(key: BgmKey, { fade = CROSSFADE_MS } = {}): void {
  ...
  void el.play().then(() => {
    ...
    // 자동재생이 막혔을 때 정적만 남는다.
    //
    // 다만 막 시작한 곡은 겹치지 않고 바로 접는다. 로그인 화면의 첫 클릭이 그렇다 —
    // 그 클릭이 자동재생 잠금을 풀어 타이틀 곡을 켜고, 같은 클릭이 마을로 넘긴다.
    // 여기서 700ms 를 겹치면 두 곡이 한 번에 뭉개져 들린다.
...
/**
 * 곡을 끈다. **화면을 떠날 때 부르지 말 것** — 다음 화면이 자기 곡을 걸면 알아서
 * 넘어간다. 여기서 끄면 화면 사이마다 정적이 생긴다. 소리를 아예 없애야 하는
 * 자리(엔딩 정적 연출 같은 것)에만 쓴다.
 */
