...
/**
 * 재료가 어디서 나오는지. 숲이랑 전투 두 곳뿐이고 그 표를 여기 한 벌만 둔다.
 *
 * 원래는 숲 표가 `camp/ForestPage.tsx` 안에, 전투 표가 `battle/BattlePage.tsx` 와
 * `scripts/sim/gameModel.ts` 에 따로 복사돼 있었다. `scripts/sim/diagnose.ts` 의
 * "얻을 수 있는 재료" 절은 아예 손으로 적은 문자열이라, 숲 표가 구역별로 갈린 뒤에도
 * "slime_extract 는 어느 드랍 테이블에도 없음"을 계속 뱉었다. 얕은 숲에서 나오는데도.
 * 밸런스를 표에서 읽어 판단하는 이상 표는 한 곳에 있어야 한다.
 */
...
export const AREA_MATERIAL_POOL: Record<string, string[]> = {
  ...
  // monster_essence 를 여기 한 자리 넣은 건 일부러 그런 거다. 목걸이·부적이 그걸
  // 요구하는데 고대 숲(21층 해금) 전에는 어디서도 안 나와서, 20층 관문을 장비로
  // 넘는 길 자체가 없었다. 고대 숲 풀에는 두 자리니까 주력은 여전히 그쪽이다.
  // ⚠️ 약초는 구역이 깊어져도 흔해야 한다. 회복 물약 다섯 종이 전부 약초를 2~3개씩
  // 먹는데, 원래는 얕은 숲 2/7 → 깊은 숲 1/10 → 고대 숲 1/9 로 오히려 드물어졌다.
  // 그래서 후반에 약초만 0 이 되고(한 판 실측), 40층부터는 회복 물약 없이 보스를
  // 만났다. 40·50층 첫 도전 승률 13%·8% 는 보스가 세서가 아니라 빈손이라서다.
  // 세 구역 모두 두 자리씩 준다.
...
/**
 * 층수별 전투 드랍 풀.
 * monster_essence 와 enhancement_stone 은 어느 전투 드랍에도 없어서 상위 아티팩트
 * 제작이랑 장비 레벨업이 통째로 막혀 있었다. 그래서 상위 층 보상에 넣었다.
 */
export function battleDropPool(floor: number): string[] {
  // ⚠️ 약초는 전 구간에 둔다. 원래 10층까지만 나왔다. 탑은 회복 물약을 쓰는 곳인데
  // 그 재료가 11층에서 끊기니, 위로 갈수록 소모는 늘고 보급은 숲 한 곳으로 좁아졌다.
  // 층이 오르면 다른 재료가 좋아지니까 약초 비중은 알아서 줄어든다.
...
export function rollBattleDrop(floor: number): { id: string; count: number }[] {
  ...
  // 관문(15·25·35·45)도 보스급으로 준다. 그 층 넘느라 쓴 물약을 돌려받아야
  // 다음 관문까지 간다. 벽 뒤에 보상이 없으면 그건 벽이 아니라 통행세다.
  //
  // ⚠️ 일반 층 드랍도 층 따라 무거워진다. 원래는 1층이든 49층이든 45% 에 1개였다.
  // 26층부터 구간 배수로 층을 길게 만들어 놓고 보급은 그대로 뒀으니, 위로 갈수록
  // 소모가 보급을 앞질렀다. 40층부터 회복 물약 0 개로 보스를 만났고.
...
/** 값이 없는 재료는 흔한 것으로 친다. 새 재료를 넣고 표를 안 고쳐도 0 으로 안 세게 */
