...
async function enterShallow(page: Page) {
  ...
  await expect(page.locator('[data-testid="forest-scene"]')).toBeVisible({ timeout: 20_000 });
}
...
async function walkOneStep(page: Page): Promise<void> {
  for (let i = 0; i < 30; i++) {
    ...
    if (await clickIfVisible(page, "forest-path-0")) { await page.waitForTimeout(1_200); return; }
    ...
    if (await clickIfVisible(page, "forest-capture-skip")) { await page.waitForTimeout(500); return; }
...
test("HUD에 현재 위험도와 획득 재료가 표시된다", async ({ page }) => {
  await enterShallow(page);
  await expect(page.locator('[data-testid="forest-alert"]')).toBeVisible();
  await expect(page.getByText(/획득 재료 \d+개/)).toBeVisible();
  await expect(page.locator('[data-testid="forest-go-home"]')).toContainText("수확 100% 회수");
});
