...
> **64×64 8방향 아틀라스**로 바뀌었고(`src/shared/playerSprite.ts` 의
> `PLAYER_FRAME_SIZE`), 표시는 베이스캠프에서 3배입니다 — 칸은 192px, 그 안에 그려진
> 인물은 165~180px 입니다. NPC 는 그 1.2배(208px)로 맞춥니다. 공방만은 방 전체가
...
> 리핑 스프라이트 16장은 64×64 8방향 아틀라스(`player.png` + `player.json`)로
> 갈렸고 원본은 저장소에서 지웠습니다.
| 나중에 | 13 | 13KB | 아이템 SVG |
...
> 지금은 **64×64 8방향 아틀라스**(`player.png` + `player.json`)로 갈렸고 원본 16장은
