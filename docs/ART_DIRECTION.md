...
| 에셋 종류 | 규격 | 화면 표시 배율 |
| --- | --- | --- |
| 플레이어/NPC 스프라이트 | 80×80 | 정수배만 (×2 = 160px) |
| 몬스터 전투 스프라이트 | 512×512 원본 | 자유 (아군 170 · 적 140 · 오름 256) |
| 몬스터 도감 일러스트 | 512×512 통일 | 자유 (픽셀 렌더링 끔) |
| 아이템 아이콘 | 32×32 | ×2 = 64px |
| 실내 배경 (공방 등) | 1536×1310 이상 | 축소만 허용 |
| 필드 배경 | 1536×2730 (베이스캠프 기준) | 축소만 허용 |

> 위 표는 2026-08-21 에 코드 기준으로 갱신했습니다. 플레이어는 64×64 4방향에서
> **80×80 8방향 아틀라스**로 바뀌었고(`src/shared/playerSprite.ts` 의
> `PLAYER_FRAME_SIZE`), 표시는 베이스캠프에서 2배(160px)입니다. 공방만은 방 전체가
> 한 화면에 들어오는 붙박이 화면이라 px 이 아니라 무대 높이 비율로 잡습니다
> (`PLAYER_DISPLAY_RATIO`).
>
> 몬스터 전투 스프라이트에 정수배 규칙을 적으면 안 됩니다. 몬스터 일러스트는
> 픽셀아트가 아니라 `image-rendering: pixelated` 를 먹이지 않기 때문입니다
> (CLAUDE.md). 실제 표시 크기는 `src/battle/battleLayout.ts` 한 벌에 있습니다.

**절대 규칙** — 픽셀아트는 축소는 되지만 **확대는 안 됩니다**. `image-rendering: pixelated`가
걸린 상태에서 1.72배 같은 비정수 확대를 하면 어떤 픽셀은 1px, 어떤 픽셀은 2px가 되어 화질이
깨져 보입니다. (지적 당시 `PLAYER_DISPLAY = 110` 이 그랬고, 지금은 정수배만 씁니다)
...
> **공방 배경 재생성**(2-2 사실 ③)은 그 뒤 새 배경 `housing_bg` 로 교체됐습니다(3-1 B).
> 다만 이 장이 지적한 것은 해상도가 아니라 **생성 품질**(안티에일리어싱으로 뭉갠 픽셀
> 격자)이었고, 새 배경이 그 기준을 만족하는지는 아직 픽셀 블록 크기로 재 보지
> 않았습니다. 그래서 완료로 덮지 않고 **재확인 대기**로 둡니다 —
> [docs/notes/roadmap.md](notes/roadmap.md) 에 한 줄 남겨 두었습니다.
...
> 교체 19장은 전부 처리됐습니다(2026-08). 공방 배경은 `housing_bg` 한 장으로,
> 리핑 스프라이트 16장은 80×80 8방향 아틀라스(`player.png` + `player.json`)로
> 갈렸고 원본은 저장소에서 지웠습니다.
| 나중에 | 13 | 13KB | 아이템 SVG |
...
- 아군은 좌하단, 적은 우상단 — **대각선 배치**. 시선이 자연스럽게 흐릅니다. (포켓몬/FF 공통)
  왼쪽→오른쪽으로 읽는 사람은 왼쪽에 선 쪽을 자기로 인식합니다. 예전엔 이 표가 반대로
  적혀 있었고, 코드도 그대로 따라가 둘이 서로 등을 지고 있었습니다.
- 스프라이트 원본은 **왼쪽 아니면 정면을 봅니다** (15장 중 왼쪽 12·정면 3, 오른쪽 0).
  그래서 뒤집는 쪽은 **왼쪽에 선 쪽**이고, 판정은 좌표에서 나옵니다
  (`battleLayout.shouldFlipX`). 아군에 `setFlipX(true)` 를 박아 두면 좌우 배치를 바꾸는
  날 서로 등집니다 — 50층 오름처럼 자리가 다른 적이 이미 있습니다.
- **HP바는 화면에서 두 번째로 큰 요소여야 합니다.** 전투에서 가장 자주 보는 정보입니다.
- HP바 색: 100~50% `moss-500` / 50~25% `ember-500` / 25% 이하 `ember-700` + 느린 맥박.
  경계값(`palette.HP_DANGER_PCT`)은 한 벌뿐입니다 — 20% 였을 때는 대개 한 대 더 맞으면
  죽는 시점이라 경고를 보고 손쓸 여지가 없었습니다.
- 게이지는 종류마다 굵기가 정해져 있습니다 (`shared/ui/StatBar`): HP·MP 12px, 경험치 8px.
  경험치는 HP보다 **한 단계 얇되 보여야** 합니다 — 6px 은 테두리에 먹혀 사라졌습니다.
- 경험치 바 색은 **변하지 않는 `mist-300`**. HP가 잔량에 따라 세 색을 오가므로, 경험치까지
  변하면 어느 쪽이 줄고 어느 쪽이 느는지 색으로 구분이 안 됩니다. 예전에 손으로 만든
  경험치 막대가 `ember-500` 이었는데, 그건 HP 위험 단계와 같은 색입니다.
- **위험 표시는 바와 몬스터가 같이** 켜집니다. 바 테두리(`ember-700`)와 몬스터 뒤 아우라가
  같은 박자로 뜁니다. 숫자만 빨개지면 아무도 안 봅니다. `prefers-reduced-motion` 이면
  맥박 없이 켜 둔 채로 둡니다 — 경고 자체를 지우지는 않습니다.
...
### 3-3. 플레이어 스프라이트 교체 (법적 필수) — **처리됨 (2026-08)**

> 당시 `public/assets/player/player-{down,up,left,right}*.png` 16장은 포켓몬 리핑
> 에셋이었고, 닌텐도는 팬게임에 예외를 두지 않으므로 배포 전 교체가 필수였습니다.
> 지금은 **80×80 8방향 아틀라스**(`player.png` + `player.json`)로 갈렸고 원본 16장은
> 저장소에서 지웠습니다. 아래 규격표는 그때 넘긴 것이고, 크기 한 칸만 실제 값으로
> 고쳐 두었습니다 — 나머지는 그대로 지켜졌습니다.
> 넣는 절차는 [docs/ASSET_HANDOFF.md](ASSET_HANDOFF.md), 출처는
...
| 항목 | 값 |
| --- | --- |
| 크기 | **80×80** (넘길 때 잡은 64×64 에서 커졌다. 표시는 정수 2배 = 160px) |
...
디자인하지 말고 저 실루엣을 확대·정면화하는 게 가장 일관성 있는 답입니다. 실제로 그렇게 갔습니다.
...
- ~~공방 배경 재생성 (이미지 생성 도구)~~ → 교체됨. **생성 품질은 재확인 대기**
- ~~플레이어 8방향 스프라이트 (PixelLab 등 — 계정·결제 필요)~~ → 80×80 아틀라스로 처리됨
- ~~아이템 아이콘 32×32 픽셀 PNG~~ → 21종 구워 넣음(`npm run build:icons`)
