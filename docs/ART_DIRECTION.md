...
이 문서는 디자인 작업의 기준 문서입니다. 디자인 관련 작업을 할 때는
`@docs/ART_DIRECTION.md 기준으로 …` 라고 이 문서를 참조하세요.
...
| `violet-500` | `#6D4E85` | 늪의 자줏빛 | 독 속성 칩 (2026-09-07 추가) |
| `rose-300`   | `#D9A0B0` | 수정이 굴절시킨 빛 | 크리스탈 속성 칩 (2026-09-07 추가) |
| `spark-400`  | `#E8CE72` | 번개의 금빛 | 전기 속성 칩 (2026-09-07 추가) |

> **속성색 셋을 왜 더 넣었나.** 속성 칩은 팔레트에 있는 색만으로 접다 보니 같은 색상환
> 자리를 명도로만 갈랐다 — 불(ember-600)과 전기(ember-500)가 한 단계 차이였고, 독은
> 보라가 없어서 흙빛(earth-500)으로 대신하고 있었다. 화면 한 칸짜리 12px 칩에서 그
> 차이는 안 보인다. 속성이 여덟으로 늘면서 색상환 자리가 모자라기도 했다.
> 이 셋은 **속성 칩과 「내 몬스터」의 속성 강조에만** 쓴다. 화면 톤을 정하는 색이 아니다.
...
@theme {
  ...
  --color-violet-500: #6D4E85;
  --color-rose-300:   #D9A0B0;
  --color-spark-400:  #E8CE72;
...
| 에셋 종류 | 규격 | 화면 표시 배율 |
| --- | --- | --- |
| 플레이어/NPC 스프라이트 | 64×64 | 정수배만 (×3 = 192px 칸) |
...
> 위 표는 2026-08-26 에 코드 기준으로 갱신했습니다. 플레이어는 4방향 리핑 에셋에서
> 8방향 아틀라스를 거쳐, 지금은 **방향마다 여섯 칸짜리 시트 한 장**입니다
> (`src/shared/playerSprite.ts` 의 `PLAYER_SHEET_METRICS`). 칸 크기가 시트마다 다르고
> 원화가 칸을 꽉 채우지 않아서, 세우는 기준은 칸의 아래가 아니라 **발이 닿는 줄**입니다.
> 표시 칸은 베이스캠프에서 192px 이고 NPC 도 같은 192px 입니다. 공방만은 방 전체가
...
> (프로젝트 규칙 문서 참고). 실제 표시 크기는 `src/battle/battleLayout.ts` 한 벌에 있습니다.
...
### 1-6. 화면 폭 대응 (2026-08-24 추가)

지원 폭은 **가로 768px 이상**입니다(`SmallScreenNotice`). 그 아래는 안내를 띄우고 막습니다.

문제는 글자를 못 늘린다는 점입니다. 픽셀 폰트라 12/24/36(Galmuri11)·16/32(Galmuri14)
정수배에 묶여 있어서, 화면이 1280 에서 1920 으로 넓어져도 글자는 그대로입니다. 그래서
**화면에 맞추는 일은 여백·칸 폭·열 수가 대신 집니다.**

`src/index.css` 에 자 한 벌이 있고, 화면은 이 이름으로만 부릅니다
(`p-panel` · `gap-gutter` · `w-rail` · `max-w-stage` …). 화면마다 `lg:`/`xl:` 분기를
따로 쓰지 않습니다 — 그러면 같은 폭에서 화면끼리 어긋납니다.

| 토큰 | 뜻 | 768+ | 1024+ | **1440 기준** | 1800+ |
| --- | --- | --- | --- | --- | --- |
| `--spacing-gutter` | 화면 가장자리 · 패널 사이 | 8 | 12 | 12 | 16 |
| `--spacing-panel` | 패널 안쪽 | 12 | 12 | 16 | 20 |
| `--spacing-stack` | 패널 안 구역 사이 | 12 | 16 | 20 | 24 |
| `--container-rail` | 좁은 세로 패널 (메뉴·파티) | 224 | 224 | 256 | 304 |
| `--container-aside` | 넓은 세로 패널 (상태창) | 264 | 288 | 320 | 384 |
| `--container-stage` | 가운데 한 기둥 (숲·정산·한 줄 모달) | 480 | 512 | 576 | 704 |
| `--container-board` | 가로로 펼쳐지는 판 (가방·큰 모달) | 768 | 896 | 1024 | 1280 |
| `--container-tile` | 격자 한 칸의 최소 폭 | 108 | 116 | 116 | 148 |

**규칙**

- **1440 이 기준**입니다. 좁으면 틀을 조여 내용에 자리를 주고, 넓으면 칸을 넓혀 빈자리를
  줄입니다. 새 값을 정할 때 1440 열을 먼저 채우고 양쪽으로 벌립니다.
- 값은 전부 4px 그리드 위에 있습니다.
- 격자는 열 수를 **박지 말고** 칸 폭이 정하게 둡니다
  (`repeat(auto-fill, minmax(var(--container-tile), 1fr))`). 고정 3/4열은 넓은 화면에서
  칸만 늘고 카드는 그대로라, 넓어진 자리가 전부 카드 사이 여백으로 갑니다.
- 확인은 `npm run design:responsive` — 768/1280/1440/1920 을 찍고, 지원 폭에서 가로
  스크롤이 생기면 실패합니다. 넓은 쪽은 `capture: monsters-full-1920` · `bag-1920`,
  좁은 쪽은 `monsters-full-1280` 을 **짝으로** 봅니다. 한 장만 보면 어느 쪽으로
  틀어졌는지 알 수 없습니다.

### 1-7. 걸어 다니는 화면의 HUD — 그림 옆 띠 (2026-08-25 추가)

베이스캠프와 공방은 **비율이 고정**입니다. 베이스캠프는 Phaser 캔버스 16:9 에 맵이
캔버스보다 좁고(1536×0.5=768 < 960), 공방은 배경 원화가 2400:1792 입니다. 창 비율이
거기 안 맞으면 둘레에 `shadow-900` 띠가 남습니다.

이 띠는 **버리는 자리가 아니라 HUD 가 사는 자리**입니다. 창 모서리(`fixed bottom-4
left-4`)에 붙이면 그림 위로 얹혀 튀어나온 것처럼 보이고, 그림 모서리에 붙이면 원화를
가립니다. 그래서 늘 떠 있는 것은 띠로 내보냅니다.

| 화면 | 좌우 띠(한쪽) | 위아래 띠(한쪽) |
| --- | --- | --- |
| 베이스캠프 1920x1080 | 192 | 0 |
| 베이스캠프 1440x900 | 144 | 45 |
| 베이스캠프 1280x720 | 128 | 0 |
| 공방 1920x1080 | 237 | 0 |
| 공방 1440x900 | 117 | 0 |
| 공방 768x1024 | 0 | 225 |
...
- 자리는 `shared/ui/StageHud`(그림 칸)와 `StageRail`(옆 띠) 두 부품이 잡습니다. 칸을
  재는 계산은 `shared/ui/stageRect.ts` 한 곳입니다 — 공방은 `containRect`(비율 하나),
  베이스캠프는 `cameraRect`(FIT 띠 + 카메라 여백 두 겹).
- **띠에 사는 것**: 메뉴, 조작 안내, 뒤로가기, 최근 제작 패널. 늘 떠 있고 장면과
  상관없는 것들입니다.
- **그림 위에 남는 것**: 목표 띠, 상호작용 안내, 공방 타이틀, NPC 대사창. 장면을
  보면서 읽는 것들이라 옆으로 빼면 눈이 두 번 움직입니다.
- 띠 폭은 창 비율이 정하므로 **패널 폭을 박지 않습니다.** `StageRail` 이
  `clamp(136px, 띠폭 − gutter×2, --container-rail)` 로 잡아 넘깁니다. 아래 136 은
  "WASD / 방향키"(12px 픽셀 폰트) 한 줄에 안쪽 여백을 더한 값입니다. 위 상한이 없으면
  넓은 화면에서 메뉴만 부풀어 다른 화면의 패널과 어긋납니다.
- 띠가 136 보다 좁은 화면(공방 768 세로)에서는 그만큼 그림 위로 걸칩니다. 원화를 조금
  가리는 쪽이 글자가 낱말마다 끊기는 쪽보다 낫습니다.
- 띠에 드는 글은 **키와 동작을 위아래로** 씁니다(`ControlHint`). 한 줄로 두면 제일 좁은
  띠에서 "WASD / 방향키" 와 "이동" 사이가 저 혼자 끊겨 두 항목으로 읽힙니다.
...
> 리핑 스프라이트 16장은 8방향 아틀라스(이후 방향별 시트)로
> 갈렸고 원본은 저장소에서 지웠습니다.
| 나중에 | 13 | 13KB | 아이템 SVG |
...
> 지금은 **방향별 시트 다섯 장**(`남·남동·동·북동·북.png`)으로 갈렸고 원본 16장은
> 저장소에서 지웠습니다. 중간에 거쳤던 64×64 아틀라스(`player.png` + `player.json`)와
> 그걸 읽던 코드도 2026-09-15 에 걷어냈습니다 — 쓰는 곳 없이 자기 테스트만 붙잡고
> 있었고, `player.png` 는 이미 없는 파일을 가리키고 있었습니다.
> 아래 규격표는 그때 넘긴 것이라 크기 칸은 지금과 다릅니다 — 나머지는 그대로 지켜졌습니다.
...
| 항목 | 값 |
| --- | --- |
| 크기 | **64×64** (표시는 정수 3배 = 192px 칸, 그 안의 인물이 165~180px) |
...
10. 검증 루프 (Playwright 캡처 / 프로젝트 규칙 / 디자인 리뷰)

**사람이 해야 하는 것**

- ~~공방 배경 재생성 (이미지 생성 도구)~~ → 교체됨. **생성 품질은 재확인 대기**
- ~~플레이어 8방향 스프라이트 (PixelLab 등 — 계정·결제 필요)~~ → 64×64 아틀라스로 처리됨
...
- **새 화면을 만들면** → 3장에 스펙을 먼저 씁니다. 그리고 작업할 때 그 절을 참조합니다.
