...
| 시트 크기 | 2048×2048 이하 (지금 320×320) |

인물 키는 55~60px 이고 **발끝은 아래에서 세 번째 줄**이다. 이 값이 곧 발밑 충돌
...
**오른쪽 넷은 배낭이 몸 왼쪽에 와야 한다.** 이게 방향이 맞는지 보는 기준이다 — 배낭이
반대쪽에 있으면 그 줄은 좌우가 뒤집혀 들어온 것이다. 실제로 한 번 그랬다(2026-09-02:
SE 줄만 SW 를 향해 그려져 있었다. 배낭 무게중심이 E −8.8 · NE −5.6 인데 SE 만 +4.0).
새 시트를 넣을 때 이 값을 재 보면 눈으로 헷갈릴 일이 없다.
...
베이스캠프는 정수배만 쓴다 — 64×64 를 **3배(192px 칸, 인물은 165~180px)** 로 그린다. 2.5배 같은 비정수
